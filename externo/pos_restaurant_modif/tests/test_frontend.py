...
class TestFrontend(odoo.tests.HttpCase):
    def test_01_pos_restaurant(self):
        ...
        pos_config = env.ref('pos_restaurant_modif.pos_config_restaurant')
        ...
        coke = self.env.ref('pos_restaurant_modif.coke')
        coke.write({'taxes_id': [(6, 0, [])]})
        water = self.env.ref('pos_restaurant_modif.water')
        water.write({'taxes_id': [(6, 0, [])]})
        minute_maid = self.env.ref('pos_restaurant_modif.minute_maid')
